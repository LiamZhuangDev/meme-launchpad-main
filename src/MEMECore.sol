    ...
    ReentrancyGuardUpgradeable
{
        ...
        returns (address tokenAddress)
    {
        ...
        if (params.saleAmount < params.totalSupply * params.initialBuyPercentage / 10000) {
            revert InvalidSaleParameters();
        }
        ...
        if (params.initialBuyPercentage > 0) {
            (initialTokens, initialBNB, adjustedBNBReserve, adjustedTokenReserve) = _calculateInitialBuy(
        ...
        // 9. 设置转账模式
        MetaNodeToken(tokenAddress).setTransferMode(MetaNodeToken.TransferMode.MODE_TRANSFER_CONTROLLED);
        ...
        if (initialTokens > 0) {
            ...
            emit TokenCreatedWithInitialBuy(
                tokenAddress, params.creator, initialTokens, initialBNB, params.initialBuyPercentage
            );
        }
     ...
     *    - 代币切换为 RESTRICTED 模式
     */
    function buy(address token, uint256 minTokenAmount, uint256 deadline)
        external
        payable
        nonReentrant
        whenNotPaused
        validToken(token)
        onlyTradingToken(token)
    {
        // 1-2. 校验
    ...
    function calculateInitialBuyBNB(
        ...
        (, uint256 bnbRequired,,) =
            _calculateInitialBuy(totalSupply, virtualBNBReserve, virtualTokenReserve, percentageBP);
    ...
    function _createVestingSchedules(
        ...
        if (tokensToVest > 0) {
            ...
            vesting.createVestingSchedules(tokenAddress, beneficiary, actualVestingAllocations);

            emit VestingCreated(tokenAddress, beneficiary, tokensToVest, actualVestingAllocations.length);
        }
    ...
    function _calculateInitialBuy(
        ...
        uint256 virtualTokenReserve,
        uint256 percentageBP
    ) internal pure returns (uint256 tokensOut, uint256 bnbRequired, uint256 newBNBReserve, uint256 newTokenReserve) {
        // 注意：这个检查是防御性的，因为调用者（createToken 和 calculateInitialBuyBNB）已经检查过了
