    ...
    ReentrancyGuardUpgradeable
{
     ...
     *    - 更新合约锁定总量
     */
    function createVestingSchedules(address token, address beneficiary, VestingAllocation[] calldata allocations)
        external
        override
        ...
        returns (uint256[] memory scheduleIds)
    {
        ...
        for (uint256 i = 0; i < allocations.length; i++) {
            ...
            emit VestingScheduleCreated(token, beneficiary, scheduleId, allocations[i].amount, startTime, endTime);
        }
    }
     ...
     *    - 将可领取代币转给调用者
     */
    function claim(address token, uint256 scheduleId)
        public
        override
        nonReentrant
        whenNotPaused
     ...
     * @notice 获取归属计划详情
     */
    function getVestingSchedule(address token, address beneficiary, uint256 scheduleId)
        external
        view
        override
     ...
     * @notice 获取用户的归属计划数量
     */
    function getVestingScheduleCount(address token, address beneficiary) external view override returns (uint256) {
     ...
     * @notice 获取所有计划的可领取总量
     */
    function getTotalClaimableAmount(address token, address beneficiary)
        external
        view
        override
    ...
    function getTotalVestedAmount(address token, address beneficiary)
        ...
        view
        override
        returns (uint256 vested, uint256 claimed, uint256 locked)
    {
        uint256 count = scheduleCount[token][beneficiary];
     ...
     * 4. 标记计划为已撤销
     */
    function revokeVestingSchedule(address token, address beneficiary, uint256 scheduleId)
        external
        override
        onlyRole(ADMIN_ROLE)
    {
     ...
     */
    function emergencyWithdrawToken(address token, uint256 amount) external override onlyRole(ADMIN_ROLE) {
